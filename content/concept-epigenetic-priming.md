...
## 한 줄 요약
발달기 경험이 크로마틴을 더 열려 있고 반응하기 쉬운 상태로 남겨, **기저 상태의 표현형(비스트레스 시 도파민 뉴런 흥분성 등)은 바꾸지 않되 훗날의 자극에 대한 전사·생리·행동 반응을 증폭**시키는 분자 기억 기전.
...
> ⚠️ '기저 차이 없음'의 근거 범위: [[kim-2026-early-life-stress-alters-h3k4me1]]에서 기저 정상이 직접 확인된 것은 **비스트레스 상태의 도파민 뉴런 흥분성**(input-output·AP threshold·입력저항 불변)이며, RNA-seq 설계에 Setd7-OE+Ctrl·Gfp+Ctrl 군이 있었음에도 기저 전사체의 OE vs Gfp 비교는 본문에 보고되지 않았다. 같은 논문에서 ELS 자체는 성체 VTA 기저에서 Setd7↑·Kmt2a↓를 남기고, 저자들도 서론에서 ELS 후 **'outright' 기저 유전자 발현 차이와 2차 스트레스로 드러나는 잠재적 변화가 병존**한다고 쓴다. [[cunningham-2026-early-life-experiences-the-neural]]는 LBN+MS 단독의 광범위한 중변연계 전사 교란(수컷 VTA·암컷 NAc에서 최대)을 정리한다. 'priming'은 **하류 반응성의 잠재화**를 뜻하지 '기저 전사 무변화'를 뜻하지 않는다.
...
- 바이러스 조작으로 **양방향 인과** 성립:
  - **Setd7-OE**(P14) → H3K4me1 34%↑(H3K4me3·H3K27Ac 불변) → 성체 스트레스에 대한 **전사 반응 방향 반전**(대조 Gfp에서 성체 스트레스 DEG의 72%가 하향[99 down / 39 up] vs 스트레스 후 Setd7-OE를 Gfp와 비교하면 DEG의 94%가 상향[48 up / 3 down]; RRHO로 Gfp에서 스트레스에 하향되던 유전자가 Setd7-OE에서는 우세 상향임을 확인 — [[kim-2026-early-life-stress-alters-h3k4me1]]), **도파민 뉴런 흥분성·I_h ↑**(비스트레스 상태에서는 무효), **사회회피·불안 취약성 ↑** — ELS 없이도 ELS 유사 표현형(**충분**).
...
- **Bottom-up LC-MS/MS 히스톤 질량분석**: 히스톤 tail을 분절해 개별 변형뿐 아니라 **같은 조각 내 조합 변형**(예: H3K9Ac-K14Ac)을 동시 정량. 항체 기반 ChIP가 놓치는 조합 상태를 잡는다.
  ...
  > ⚠️ 단, [[kim-2026-early-life-stress-alters-h3k4me1]]의 MS는 bulk VTA 조직이다. [[concept-glia-stress-plasticity]]의 세포 구성 규칙에 따라 '75% permissive' 지형은 세포 내재적 재편으로 단정하지 않는다(세포 수준 근거는 TH⁺ 핵 SETD7 IHC와 바이러스 인과 실험).
...
- **섭식으로의 이식**: [[shin-2023-early-adversity-promotes-binge-like-eating]]의 ELS→LH^Lepr 폭식 회로도 동일한 two-hit 구조를 갖는다. 그 회로에 **크로마틴 priming 층이 있는지는 이 위키의 자료 범위 내에서 검증되지 않았다** — 직접 물어볼 만한 공백.
  ...
  > ⚠️ 창 불일치: Shin 2023의 ELS는 **P3–4 단회 23h 모성분리**, Kim 2026은 **P10–17 만성 LBN**이다. [[cunningham-2026-early-life-experiences-the-neural]]는 PND 2–9와 PND 10–17/20 노출을 다른 표현형군으로 분류한다. 'Shin 회로에 크로마틴 priming 층이 있는가'를 물을 때는 어느 창의 ELS로 물을지를 먼저 고정해야 하며, Setd7 소견을 P3–4 모델로 옮기는 것은 외삽이다.
